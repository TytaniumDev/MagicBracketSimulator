...
## 2025-02-18 - Argument Injection in Docker Commands

**Vulnerability:** Unsanitized variables (`containerName`) were passed to `docker rm` and `docker inspect` using `child_process.execFile` and `spawn`.

**Learning:** If an input string starts with a hyphen (`-`), command-line tools parse it as an argument/flag instead of a positional parameter. An attacker could potentially achieve argument injection or cause unpredictable behavior.

**Prevention:** Use the `--` (end-of-options) separator before untrusted positional arguments in command-line executions (e.g. `['rm', '-f', '--', containerName]`). Always sanitize IDs used in commands or paths using strict regex formats (e.g., `/^[a-zA-Z0-9_-]+$/`).
