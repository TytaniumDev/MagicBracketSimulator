...
- **[2026-10-24]**: Moved lint and test jobs from external reusable workflows to local shared scripts (`scripts/lint.sh`, `scripts/test.sh`) in `.github/workflows/ci.yml` to enforce true environment parity. Created `docs/TESTING.md` to extract the bloat from `AGENTS.md` and explicitly detailed command paths using `cd`.
