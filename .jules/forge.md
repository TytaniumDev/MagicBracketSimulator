...
- **[2024-05-18]**: Extracted testing commands from `AGENTS.md` to a domain-specific file `docs/TESTING.md` to reduce root-level bloat and adhere to standard length constraints. Added frontend `vitest` command (`npm run test --prefix frontend`) to `scripts/test.sh` for parity with CI execution.
