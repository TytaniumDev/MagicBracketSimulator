...
- **[2026-02-10]**: Found that `orchestrator-service` unit tests for `auth` and `ingestion` were not running in CI. Updated `test:unit` script to chain `game-logs`, `ingestion`, and `auth` tests. Calibrated `AGENTS.md` to reflect this and clarified deployment triggers.
