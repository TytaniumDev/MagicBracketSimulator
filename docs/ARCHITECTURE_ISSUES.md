...
**Mitigation:** Add zod schemas for request bodies at system boundaries. Internal service-to-service calls (worker → API) still benefit from validation since unexpected responses or retries can send malformed payloads.
...
1. **No state machine enforcement** (resolved in PR #78) — Invalid state transitions from HTTP polling retries caused COMPLETED simulations to revert to RUNNING.
